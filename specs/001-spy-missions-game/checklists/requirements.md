...
1. **No [NEEDS CLARIFICATION] markers remain** — FAIL
	...
	- "**FR-006**: ... [NEEDS CLARIFICATION: game end condition — fixed duration (days), first-to-X points, or player agreement?]"
...
**What we need to know**: Confirm the intended resource regeneration cadence for games and whether the value should be configurable for testing/game modes.
...
| B | Every 24 hours per game start time | Consistent interval per game; easier to reason about for timed games. |
...
## Question 2: Game end condition

**Context**: Quote from spec: "**FR-006**: System MUST compute and persist victory points ... declare a winner when the game end condition is met. [NEEDS CLARIFICATION: game end condition — fixed duration (days), first-to-X points, or player agreement?]"
...
| Option | Answer | Implications |
|--------|--------|--------------|
| A | Fixed duration (e.g., N days) | Predictable game length; scoring at end. Easy to schedule. |
| B | First-to-X victory points | Games end when a player reaches a target; may be faster and more competitive. |
